                    ...
                    <div class="card custom-card">
                        <div class="card-header justify-content-between">
                            ...
                            <div class="d-flex">
                                ...
                                <a href="{{url('admin/createtestimonial')}}"
                                    class="btn btn-sm btn-primary btn-wave waves-light"><i
                                        class="ri-add-line fw-medium align-middle me-1"></i> Create Testimonial</a>
                            </div>
                        </div>
                        <div class="card-body p-0">
                            <div class="table-responsive">
                                <table class="table text-nowrap">
                                    <thead>
                                        <tr class="testimonial-list">
                                            <th class="bg-yellow-themed text-fixed-white " scope="col">S.No.</th>
                                            <th class="bg-yellow-themed text-fixed-white " scope="col">Testimonial Name
                                            </th>
                                    ...
                                    <tbody id="dataTable">
                                        @foreach($testimonial as $index => $cat)
                                        <tr class="testimonial-list">
                                            <td>{{ $index + 1 }}</td>
                                            <td class="data-name">{{ $cat->name }}</td>
                                            <td>
                                                <a href="{{ route('edittestimonial', $cat->id) }}"
                                                    class="btn btn-primary-light btn-icon btn-sm">
                                                    <i class="ri-edit-line"></i>
                                                </a>
                                                <form action="{{ route('testimonial.destroy', $cat->id) }}" method="POST"
