...
@section('page_title','View Municipality')
@push("after-styles")
                    ...
                    <div class="card custom-card">
                        <div class="card-header justify-content-between">
                            ...
                            <div class="d-flex">
                                ...
                                <a href="{{url('admin/createmunicipality')}}"
                                    class="btn btn-sm btn-primary btn-wave waves-light"><i
                                        class="ri-add-line fw-medium align-middle me-1"></i> Create Municipality</a>
                                ...
                                <table class="table text-nowrap">
                                    <thead>
                                        <tr class="Municipality-list">
                                            ...
                                            <th class="bg-yellow-themed text-fixed-white " scope="col">Municipality Name
                                            </th>
                                            <th class="bg-yellow-themed text-fixed-white " scope="col">Short Name
                                            </th>
                                            <th class="bg-yellow-themed text-fixed-white " scope="col">District
                                            </th>
                                            <th class="bg-yellow-themed text-fixed-white " scope="col">Action</th>
                                        </tr>
                                    </thead>
                                    <tbody id="dataTable">
                                        @foreach($municipality as $index => $muni)
                                        <tr class="Municipality-list">
                                            <td>{{ $index + 1 }}</td>
                                            <td class="muni-name">{{ $muni->municipality_name }}</td>
                                            <td class="data-name">{{ $muni->municipality_shortname }}</td>
                                            <td class="province-name">{{ $muni->district->district_name}}</td>
                                            <td>
                                                <a href="{{ route('editmunicipality', $muni->id) }}"
                                                    class="btn btn-primary-light btn-icon btn-sm">
                                                    <i class="ri-edit-line"></i>
                                                </a>
                                                <form action="{{ route('municipality.destroy', $muni->id) }}"
