...
@section('page_title','View Facility')
@push("after-styles")
                    ...
                    <div class="card custom-card">
                        <div class="card-header justify-content-between">
                            ...
                            <div class="d-flex">
                                ...
                                <a href="{{url('admin/createfacility')}}"
                                    class="btn btn-sm btn-primary btn-wave waves-light"><i
                                        class="ri-add-line fw-medium align-middle me-1"></i> Create Facility</a>
                                ...
                                <table class="table text-nowrap">
                                    <thead>
                                        <tr class="category-list">
                                            ...
                                            <th class="bg-yellow-themed text-fixed-white " scope="col">Facility Name
                                            </th>
                                            <th class="bg-yellow-themed text-fixed-white " scope="col">Facility Logo
                                            </th>
                                            <th class="bg-yellow-themed text-fixed-white " scope="col">Action</th>
                                        </tr>
                                    </thead>
                                    <tbody id="dataTable">
                                        @foreach($facility as $index => $fat)
                                        <tr class="category-list">
                                            <td>{{ $index + 1 }}</td>
                                            <td class="data-name">{{ $fat->facility_name }}</td>
                                            <td><img src="{{asset('/uploads/iconsfacility/'.$fat->facility_logo)}}"
                                                    alt=""> </td>
                                            <td>
                                                <a href="{{ route('editfacility', $fat->id) }}"
                                                    class="btn btn-primary-light btn-icon btn-sm">
                                                    <i class="ri-edit-line"></i>
                                                </a>
                                                <form action="{{ route('facility.destroy', $fat->id) }}" method="POST"
