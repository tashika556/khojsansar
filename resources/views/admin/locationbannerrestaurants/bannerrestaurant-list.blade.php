...
@section('page_title','View Banner for Restaurant Location')
@push("after-styles")
            ...
            <div class="my-4 page-header-breadcrumb d-flex align-items-center justify-content-between flex-wrap gap-2">
                <div>
                    ...
                    <ol class="breadcrumb mb-0">
                        ...
                        <li class="breadcrumb-item"><a href="{{url('admin/locationrestaurant')}}">Banner for Restaurant
                                Location</a></li>
                    ...
                    <div class="card custom-card">
                        <div class="card-header justify-content-between">
                            <div class="card-title">
                                Manage Banner for Restaurant Location Details <span class="text-danger"> ***Remember:
                                    You can only upload upto 5 items. So, upload the banners
                                    for the main district location as your requirement
                                </span>
                            </div>
                            @include('admin.auth.error')
                            <div class="d-flex">
                                <a href="{{url('admin/createlocationrestaurant')}}"
                                    class="btn btn-sm btn-primary btn-wave waves-light" id="createBannerButton" ><i
                                        class="ri-add-line fw-medium align-middle me-1"></i> Create Banner for
                        ...
                        <div class="card-body p-0">
                            <div class="table-responsive">
                                <table class="table text-nowrap">
                                    <thead>
                                        <tr class="bannerbyrestaurant-list">
                                            <th class="bg-yellow-themed text-fixed-white " scope="col">S.No.</th>
                                            <th class="bg-yellow-themed text-fixed-white " scope="col">District Name
                                            </th>
                                    ...
                                    <tbody id="dataTable">
                                        @foreach($locationrestaurant as $index => $cat)
                                        <tr class="bannerbyrestaurant-lists">
                                            <td>{{ $index + 1 }}</td>
                                            <td class="data-name">{{ $cat->district->district_name }}</td>
                                            <td>
                                                <a href="{{ route('editlocationrestaurant', $cat->id) }}"
                                                    class="btn btn-primary-light btn-icon btn-sm">
                                                    <i class="ri-edit-line"></i>
                                                </a>
                                                <form action="{{ route('locationrestaurant.destroy', $cat->id) }}"
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        const createButton = document.getElementById('createBannerButton');
        const maxItems = 5;
        const currentItems = document.querySelectorAll('.bannerbyrestaurant-lists').length;

        createButton.addEventListener('click', function (event) {
            if (currentItems >= maxItems) {
                event.preventDefault(); 
                alert("You can only upload up to 5 items.");
            }
        });
    });
</script>
