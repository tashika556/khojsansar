...
@section('page_title','View Payment done by Business')
@push("after-styles")
            ...
            <div class="my-4 page-header-breadcrumb d-flex align-items-center justify-content-between flex-wrap gap-2">
                <div>
                    ...
                    <ol class="breadcrumb mb-0">
                        ...
                        <li class="breadcrumb-item"><a href="{{url('admin/businesspayment')}}">Payment done by Business</a>
                        </li>
                    ...
                    <div class="card custom-card">
                        <div class="card-header justify-content-between">
                            <div class="card-title">
                                Manage Payment done by Business
                            </div>
                        ...
                        <div class="card-body p-0">
                            <div class="table-responsive">
                                <table class="table text-nowrap">
                                    ...
                                    <tbody id="dataTable">
                                        @foreach($payment as $index => $bus)
                                        <tr class="Business-list">
                                            <td>{{ $index + 1 }}</td>
                                            <td class="data-name">{{ $bus->businesses->customershow->business }}</td>
                                            <td class="province-name">
                                                {{ $bus->businesses->customershow->categoryshow->category_name }}</td>
                                            <td class="muni-name">{{ $bus->businesses->customershow->first_name}}
                                                {{ $bus->businesses->customershow->middle_name}} {{ $bus->businesses->customershow->last_name}}
                                            </td>
                                            <td>

                                                {{ $bus->payment_confirmation ? 'Yes' : 'No' }}
                                            </td>
                                            <td>
                                                <a href="{{ route('viewbusinesspayment', $bus->id) }}" class="btn btn-info btn-icon btn-sm">
